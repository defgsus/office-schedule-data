...
class TimespanChecker:
    def __init__(self, dates_dt: List[datetime.datetime]):
        self.dates_dt = dates_dt
        self.ts_conv_funcs = {}
        self.ts_check_funcs = {}
        for ts in METRIC_TIMESPANS:
            value = int("".join(c for c in ts if c.isnumeric()))

            if ts.startswith("dist_"):
                if ts.endswith("h"):
        ...
        self.dates_ts = {
            ts: self.dates_dt if not self.ts_conv_funcs.get(ts)
                else [self.ts_conv_funcs[ts](d) for d in self.dates_dt]
            for ts in METRIC_TIMESPANS
        }

    def test_row(self, dt: datetime.datetime, is_free: List) -> Dict[str, int]:
        dt_ts = {key: func(dt) for key, func in self.ts_conv_funcs.items()}
        ret = {ts: 0 for ts in METRIC_TIMESPANS}
        for i, free in enumerate(is_free):
            if free:
                for ts in METRIC_TIMESPANS:
                    d1 = dt_ts.get(ts, dt)
                    d2 = self.dates_ts[ts][i]
                    if self.ts_check_funcs[ts](d1, d2):
                        ret[ts] = ret[ts] + 1

        return ret

    def test_date(self, d1: datetime.datetime, d2: datetime.datetime) -> Dict[str, int]:
        d1_ts = {key: func(d1) for key, func in self.ts_conv_funcs.items()}
        d2_ts = {key: func(d2) for key, func in self.ts_conv_funcs.items()}

        ret = dict()
        for ts in METRIC_TIMESPANS:
            date1 = d1_ts.get(ts, d1)
            date2 = d2_ts.get(ts, d2)
            if self.ts_check_funcs[ts](date1, date2):
                ret[ts] = 1
        return ret
    ...
    @classmethod
    def within_hour(cls, hour: int, equal: bool, d1: datetime.datetime, d2: datetime.datetime):
        hours = (d2 - d1).total_seconds() / 60 / 60
        return hours == hour if equal else hours <= hour

    @classmethod
    def within_day(cls, day: int, equal: bool, d1: datetime.datetime, d2: datetime.datetime):
        days = (d2 - d1).total_seconds() / 60 / 60 / 24
        return days == day if equal else days <= day
...
def calc_metrics(
    ...
    for week, source_id, columns, rows in tqdm(data.iter_tables()):
        ...
        timespan_checker = TimespanChecker(dates_dt)
        ...
        for org_row in rows:
            ...
            num_cancellations = 0
            num_free_dates = 0
            num_free_dates_ts = timespan_checker.test_row(timestamp_dt, date_row)
                        ...
                        if do_calc_booking:
                            if date != min(*cur_free_dates[loc_id]) and date != max(*cur_free_dates[loc_id]):
                                ...
                                num_appointments_ts = {
                                    ...
                                    for ts, v in timespan_checker.test_date(timestamp_dt, to_datetime(date)).items()
                                }
            ...
            for buckets, value in (
                ...
                for ts in METRIC_TIMESPANS
                    ...
                    (metrics[f"free_dates_{ts}"], num_free_dates_ts[ts])
                    for ts in METRIC_TIMESPANS
            ):
